...
const SectionFour = () => {
  ...
  useEffect(() => {
    if (technologyRefNo) {
      ...
      axios
        .get(`${BASE_URL}/api/section-four/${technologyRefNo}`, {
        ...
        .then((response) => {
          const formattedData = (response.data || []).map((deployment) => ({
            ...deployment,
            country:
              countryOptions.find((opt) => opt.value === deployment.country) ||
              null,
          }));
          setDeploymentEntries(formattedData);
        })
  ...
  const handleSubmitAll = async () => {
    if (editingIndex !== null) {
      ...
      return false; // return false so submit flow can check
    }
    ...
    try {
      const token = localStorage.getItem("token");
      const response = await axios.post(
        `${BASE_URL}/api/section-four/save/${technologyRefNo}`,
        payload,
        {
          headers: {
      ...
      setDeploymentEntries(formattedResponse);
      setEditingIndex(null);
      Swal.fire("Saved!", "Deployment details saved successfully.", "success");
      return true;
    } catch (error) {
      console.error("Save error:", error.response?.data || error.message);
      Swal.fire(
        "Error!",
        error.response?.data?.message || "Saving Section 4 failed.",
        "error"
      );
      return false;
    }
  ...
  const handleSubmitForReview = async () => {
    if (editingIndex !== null) {
      Swal.fire("Update Pending", "Finish editing before submitting.", "warning");
      return;
    }

    // If entries exist, auto-save before submitting
    if (deploymentEntries.length > 0) {
      const saved = await handleSubmitAll();
      if (!saved) return; // abort if save failed
    } else {
      // No entries — confirm user wants to proceed
      const proceed = await Swal.fire({
        title: "No Deployments Added",
        text: "Section 4 has no deployment entries. Submit anyway?",
        icon: "warning",
        showCancelButton: true,
    ...
    const confirm = await Swal.fire({
      ...
      html: `
        <p class="text-sm text-slate-600">TRN: <strong>${technologyRefNo}</strong></p>
        <p class="text-sm text-slate-500 mt-2">
          Once submitted, the admin will review and either
          <strong>approve</strong>, <strong>request revisions</strong>,
          or <strong>reject</strong> your technology.
        </p>
      `,
      icon: "question",
      showCancelButton: true,
    ...
    try {
      const token = localStorage.getItem("token");
      await axios.post(
        `${BASE_URL}/api/admin/technologies/${technologyRefNo}/submit-for-review`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      await Swal.fire({
        ...
        html: `
          <p class="text-sm text-slate-600">
            Your technology <strong>${technologyRefNo}</strong> has been
  ...
  // ── Add or update an entry in local state ─────────────────────────────────
  const handleAddOrUpdateDeployment = async (
    values,
    ...
    if (editingIndex !== null) {
      ...
      Swal.fire(
        "Updated",
        "Details updated in the list. Click 'Save' to persist.",
        "success"
      );
    } else {
      setDeploymentEntries((prev) => [...prev, deploymentData]);
      Swal.fire(
        "Added",
        "Details added to the list. Click 'Save' to persist.",
        "success"
      );
    }
  ...
  const handleRemoveDeployment = (indexToRemove, resetForm) => {
    ...
    Swal.fire({
      title: "Are you sure?",
      text: `Remove entry for '${entryToRemove.clientName || "New Entry"}'? Will be deleted on Save.`,
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#dc2626",
      cancelButtonColor: "#4b5563",
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire(
          "Removed!",
          "Removed from list. Click 'Save' to finalize.",
          "info"
        );
      }
    });
    ...
    return (
      <>
        ...
        <div className="min-h-screen bg-gradient-to-br from-sky-100 via-orange-50 to-sky-200 flex items-center justify-center">
          <p className="text-center text-slate-600">Loading deployment data…</p>
        </div>
        <FooterBar />
      </>
    );
          ...
          <div className="flex-1 px-4 py-8 md:px-8">
            ...
            >
              {({
                ...
                <>
                  {/* ── Existing deployment entries list ────────────────── */}
                  {deploymentEntries.length > 0 && (
                    <div className="mb-6 p-4 rounded-2xl border border-indigo-100 bg-gradient-to-br from-sky-50 to-indigo-100 shadow-md">
                      <h3 className="text-2xl font-extrabold mb-4 text-blue-900 flex items-center">
                        <svg
                          ...
                          >
                            <div className="flex-1 mb-3 sm:mb-0">
                              <div className="flex items-center mb-1 flex-wrap gap-2">
                                ...
                                {editingIndex === index && (
                                  <span className="bg-blue-100 text-blue-800 text-xs font-semibold px-2.5 py-0.5 rounded-full animate-pulse">
                                    Editing…
                                  </span>
                                )}
                              </div>
                            ...
                            <div className="flex space-x-2">
                              <button
                                type="button"
                                onClick={() =>
                                  handleEditLoad(deployment, index, setValues)
                                }
                                disabled={
                                  editingIndex !== null && editingIndex !== index
                                }
                                className="bg-amber-500 text-white px-4 py-2 rounded-full text-xs md:text-sm font-semibold hover:bg-amber-600 transition disabled:opacity-50 disabled:cursor-not-allowed"
                              >
                                {editingIndex === index ? "Editing" : "Edit"}
                              </button>
                              <button
                                ...
                                disabled={editingIndex === index}
                                className="bg-rose-500 text-white px-4 py-2 rounded-full text-xs md:text-sm font-semibold hover:bg-rose-600 transition disabled:opacity-50 disabled:cursor-not-allowed"
                              >
                                Remove
                              </button>
                            </div>
                  ...
                  >
                    ...
                    {/* ── Main navigation buttons ──────────────────────── */}
                    <div className="flex flex-wrap justify-between items-center gap-3 mt-8 border-t border-slate-200 pt-5">
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        {/* 💾 Save — persists to DB, no status change */}
                        <button
                          type="button"
                          onClick={handleSubmitAll}
                          disabled={editingIndex !== null || isSubmitting}
                          className="inline-flex items-center gap-2 rounded-full bg-emerald-600 px-6 py-2.5 text-xs md:text-sm font-semibold text-white hover:bg-emerald-700 disabled:opacity-60 shadow-md shadow-emerald-400/40 transition"
                        >
                          💾 Save
                        </button>

                        {/* 👁 Preview & Edit — opens ReviewPopup */}
                        <button
                          type="button"
                          onClick={handlePreviewOpen}
                          disabled={editingIndex !== null || isSubmitting}
                          className="inline-flex items-center gap-2 rounded-full bg-teal-600 px-6 py-2.5 text-xs md:text-sm font-semibold text-white hover:bg-teal-700 disabled:opacity-60 transition"
                        >
                          👁 Preview &amp; Edit
                        </button>

                        {/* 🚀 Submit for Review — saves then sends to admin */}
                        <button
                          type="button"
                          onClick={handleSubmitForReview}
                          disabled={editingIndex !== null || isSubmitting}
                          className="inline-flex items-center gap-2 rounded-full bg-indigo-600 px-6 py-2.5 text-xs md:text-sm font-semibold text-white hover:bg-indigo-700 disabled:opacity-60 shadow-md shadow-indigo-400/40 transition"
                        >
                          {isSubmitting ? (
                            <>
