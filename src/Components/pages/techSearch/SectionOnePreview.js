...
const SectionOnePreview = ({ data }) => {
    ...
    // fetch("http://localhost:8080/download/example.pdf")
    //     .then(res => res.blob())
    //     .then(blob => {
              ...
              <table>
                <tbody>
                  ...
                  <tr><th>Environmental Statutory</th><td>${item.environmentalStatutory || '-'}</td></tr>
                    <tr><th>File</th><td><img src="http://localhost:8080/apf/tdmp/images/${item.technologyRefNo}" alt='xyz' style="width: 200px; height: 200px; margin-top: 10px;"/></td></tr>
                ...
                {data.sectionOneList.map((item, index) => (
                    <div key={index} className="border p-4 rounded-lg shadow-md bg-white">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                            ...
                            <div><strong>File:</strong> <img src={`http://localhost:8080/apf/tdmp/images/${item.technologyRefNo}`} alt='xyz' style={{ width: '200px', height: '200px', marginTop: '10px' }}/></div>
