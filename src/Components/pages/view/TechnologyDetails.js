...
const TechnologyDetails = () => {
        ...
        const fetchAllData = async () => {
            ...
            try {
                ...
                const [res1, res2, res3, res4] = await Promise.all([
                    axios.get(`http://172.16.2.246:8282/api/section-one/${trnNo}`, { headers }).catch(err => { console.error("Error fetching Section 1:", err); return { data: null }; }),
                    axios.get(`http://172.16.2.246:8282/api/section-two/${trnNo}`, { headers }).catch(err => { console.error("Error fetching Section 2:", err); return { data: null }; }),
                    axios.get(`http://172.16.2.246:8282/api/section-three/${trnNo}`, { headers }).catch(err => { console.error("Error fetching Section 3:", err); return { data: [] }; }),
                    axios.get(`http://172.16.2.246:8282/api/section-four/${trnNo}`, { headers }).catch(err => { console.error("Error fetching Section 4:", err); return { data: [] }; }),
                ]);
    ...
    const SectionCard = ({ title, icon, editRoute, children }) => (
                    ...
                    {sectionOneData ? (
                        <div className="overflow-hidden rounded-2xl bg-white shadow-xl printable-card">
                            ...
                            <div className="px-6 py-5">
                                ...
                                {/* ── Images grid ── */}
                                {sectionOneData?.imageUrls?.length > 0 && (
                                    <div className="mt-6">
                                        <h3 className="text-sm font-semibold text-gray-700 mb-3">
                                            Technology Images ({sectionOneData.imageUrls.length})
                                        </h3>
                                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                                            {sectionOneData.imageUrls.map((url, i) => (
                                                <div
                                                    key={i}
                                                    className="relative group rounded-xl overflow-hidden border border-slate-200 aspect-square cursor-pointer shadow-sm hover:shadow-md transition-shadow"
                                                    onClick={() => { setFileToView(url); setIsModalOpen(true); }}
                                                >
                                                    <img
                                                        src={url}
                                                        alt={`Technology image ${i + 1}`}
