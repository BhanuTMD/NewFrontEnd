...
const OTPLoginVerify = () => {
  // ✅ FIX 1: otp stored as array of 6 strings, not a single string
  ...
  // ✅ FIX 2: handleChange works on array copy — no stale digits bleed through
  const handleChange = (e, index) => {
    const value = e.target.value;
    if (/^\d?$/.test(value)) {
      const newOtp = [...otp];         // copy the array
  ...
  // ✅ FIX 3: handleKeyDown uses array index correctly
  ...
  const handleSubmit = async (event) => {
    ...
    // ✅ FIX 4: join array to get the string, validate properly
    ...
    try {
      ...
      // ✅ FIX 5: send otpString (joined array), not raw otp state
      ...
      await Swal.fire({
        ...
        icon: "success",
        background:
          "linear-gradient(135deg, #0b1c47 0%, #1e3a8a 40%, #ff8c00 100%)",
        color: "#fff",
        iconColor: "#ffae42",
      ...
      // ✅ FIX 6: clear all OTP boxes on error so stale digits cannot persist
      setOtp(Array(6).fill(""));
      inputRefs.current[0]?.focus();
  ...
  const handleResend = async () => {
    try {
      await axios.post(
        "http://172.16.2.246:8282/api/auth/login",
    ...
    // ✅ FIX 7: also clear boxes on resend so user starts fresh
    setOtp(Array(6).fill(""));
    setErrorMsg("");
    inputRefs.current[0]?.focus();
  };
    ...
    <div className="relative min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-900 via-slate-950 to-indigo-900 px-4 overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-lg">
            ...
            <div className="flex justify-between gap-2 sm:gap-3">
              {Array.from({ length: 6 }).map((_, index) => (
                <input
                  ...
                  // ✅ FIX 8: value reads from array index — always reflects true state
                  value={otp[index]}
        ...
        <p className="mt-4 text-[11px] sm:text-xs text-center text-slate-400">
          For your security, this OTP will expire in 5 minutes. Do not share
          it with anyone.
        </p>
      </div>
    </div>
...
//       const res = await axios.post(
//         `http://172.16.2.246:8282/api/auth/login/verify-otp?email=${encodeURIComponent(
//           email
